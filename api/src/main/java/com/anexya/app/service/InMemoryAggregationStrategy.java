...
public class InMemoryAggregationStrategy implements AggregationStrategy {
    ...
    List<com.anexya.app.domain.TagRead> filtered = repository.findAll().stream()
        .filter(t -> !t.getReadAt().isBefore(startDate) && !t.getReadAt().isAfter(endDate))
        .filter(t -> siteName.map(s -> s.equals(t.getSiteName())).orElse(true))
        .filter(t -> epc.map(e -> e.equals(t.getEpc())).orElse(true))
        .toList();
    ...
    return byEpc.entrySet().stream()
        .map(entry -> {
            ...
            String mostDetectedLocation = txs.stream()
                ...
                .orElse(null);
            Instant firstSeen = txs.stream().map(com.anexya.app.domain.TagRead::getReadAt).min(Comparator.naturalOrder()).orElse(null);
            Instant lastSeen = txs.stream().map(com.anexya.app.domain.TagRead::getReadAt).max(Comparator.naturalOrder()).orElse(null);
            return new TagSummary(tag, total, avgRssi, peakRssi, locationCount, mostDetectedLocation, firstSeen, lastSeen);
        })
        .toList();
    }
}
