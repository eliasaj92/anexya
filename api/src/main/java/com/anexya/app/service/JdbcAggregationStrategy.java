...
public class JdbcAggregationStrategy implements AggregationStrategy {
    ...
    sql.append("from tag_reads rt where read_at between ? and ? ");
        ...
        return jdbcTemplate.query(sql.toString(), params.toArray(), (rs, rowNum) -> new TagSummary(
                rs.getString("epc"),
                rs.getLong("totalReadCount"),
                rs.getDouble("averageRssi"),
                rs.getDouble("peakRssi"),
                rs.getLong("locationCount"),
                rs.getString("mostDetectedLocation"),
                rs.getTimestamp("firstSeen").toInstant(),
