...
public class AwsKeyManagementService implements KeyManagementService
{
    ...
    public byte[] encrypt(byte[] plaintext, String keyId)
    {
        try
        {
            var response = kmsClient.encrypt(
                    EncryptRequest.builder().keyId(keyId).plaintext(SdkBytes.fromByteArray(plaintext)).build());
    ...
    public byte[] decrypt(byte[] ciphertext, String keyId)
    {
        try
        {
            var response = kmsClient.decrypt(
                    DecryptRequest.builder().keyId(keyId).ciphertextBlob(SdkBytes.fromByteArray(ciphertext)).build());
